...
const KNOWN_HEADERS = new Set([
  'PROFESSIONAL PROFILE','SKILLS','PROFESSIONAL EXPERIENCE','WORK EXPERIENCE',
  'OTHER EXPERIENCE','PROFESSIONAL ORGANIZATIONS','PROFESSIONAL ORGANIZATION',
  'EDUCATION','ACHIEVEMENTS','CERTIFICATIONS','SUMMARY',
...
function preprocessLLMOutput(raw: string): string {
  ...
  const isDate = (s: string) =>
    /\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{4}|present)/i.test(s) &&
    /\d{4}|present/i.test(s);
...
const profProfileHeader = () => new Paragraph({ alignment: AlignmentType.CENTER, spacing: { ...SS, before: 240, after: 0 }, children: [new TextRun({ text: 'PROFESSIONAL PROFILE', bold: true, size: S14, font: FONT })] });
...
interface Parsed {
  ...
  orgsContent: string; achievements: string[];
  education: EduBlock[];
...
function splitJobLine(line: string): { jobTitle: string; dates: string } {
  const m = line.match(/^(.+?)\s{2,}(\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec).+|present.+|\d{4}.+)$/i);
...
function parseResume(raw: string): Parsed {
  ...
  const result: Parsed = {
    ...
    skillsContent: '', experiences: [], otherExperiences: [],
    orgsContent: '', achievements: [], education: [],
    variationsRaw: [], finalNotesRaw: [],
  };

  type Sec = 'HEADER'|'PROFILE'|'SKILLS'|'EXP'|'OTHER'|'ORGS'|'ACHIEVE'|'EDU'|'VARIATIONS'|'NOTES';
  let sec: Sec = 'HEADER';
  let profileState = 0;
  const buckets: Record<string, string[]> = { SKILLS:[], EXP:[], OTHER:[], ORGS:[], ACHIEVE:[], EDU:[], VARIATIONS:[], NOTES:[] };
  ...
  for (const raw of lines) {
    ...
    if (/^PROFESSIONAL ORGANIZATIONS?$/.test(t)){ sec = 'ORGS'; continue; }
    if (t === 'ACHIEVEMENTS')                   { sec = 'ACHIEVE'; continue; }
    if (t === 'EDUCATION')                      { sec = 'EDU'; continue; }
    if (/^ADDITIONAL PROFESSIONAL PROFILE/.test(t)) { sec = 'VARIATIONS'; continue; }
    if (/^FINAL NOTES/.test(t))                 { sec = 'NOTES'; continue; }
...
function buildDoc(p: Parsed): Document {
  ...
  if (p.achievements.length > 0) {
    ...
    paras.push(blank());
    for (const a of p.achievements) paras.push(bulletP('• ' + a.replace(/^[•\-\*]\s*/, '')));
  }
  ...
  if (p.variationsRaw.length > 0 || p.finalNotesRaw.length > 0) {
    ...
    if (p.variationsRaw.length > 0) {
      paras.push(centeredBoldP('Additional Professional Profile Variations'));
      paras.push(blank());
      for (const line of p.variationsRaw) paras.push(leftP(line));
    }
    if (p.finalNotesRaw.length > 0) {
      paras.push(blank());
      paras.push(centeredBoldP('Final Notes / Rationale'));
      paras.push(blank());
      for (const line of p.finalNotesRaw) paras.push(leftP(line));
    }
  }
